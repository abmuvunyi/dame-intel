  ...
  private getValidNormalMovesForPiece(pos: Position, piece: Piece): Move[] {
    ...
    if (piece.type === PieceType.KING) {
      if (this.rules.boardSize === 10) {
        ...
        // 8x8 Kings move exactly 1 step in all 4 directions
        for (const dir of dirs) {
  ...
  private getValidJumpsForPiece(start: Position, piece: Piece, currentPos: Position = start, capturedSoFar: Position[] = []): Move[] {
    ...
    if (piece.type === PieceType.KING) {
      if (this.rules.boardSize === 10) {
        ...
        // 8x8 King captures (jump exactly 1 adjacent piece, land immediately behind)
        for (const dir of dirs) {
      ...
      // Men captures
      // In 10x10, men can capture backwards (all 4 dirs). In 8x8, only forward captures.
      ...
      for (const dir of captureDirs) {
        ...
        if (overPiece && overPiece.color !== piece.color && landPos === null && !alreadyCaptured) {
          ...
          // 8x8 promotion rule: if a man reaches the promotion row during a jump, it promotes to a king and its turn ends
