...
export enum GameVariant {
  STANDARD = 'STANDARD', // 8x8, standard rules (no flying kings, no backward capture for men)
  INTERNATIONAL = 'INTERNATIONAL' // 10x10, international rules (flying kings, backward capture for men)
}
...
export class DraughtsEngine {
  ...
  constructor(rules: Partial<GameRules> = {}) {
    const isInternational = rules.boardSize === 10;
    this.rules = {
      boardSize: rules.boardSize || 8,
      forceMajorityCapture: rules.forceMajorityCapture !== undefined ? rules.forceMajorityCapture : true,
      variant: rules.variant || (isInternational ? GameVariant.INTERNATIONAL : GameVariant.STANDARD)
    };
  ...
  private getValidNormalMovesForPiece(pos: Position, piece: Piece): Move[] {
    ...
    if (piece.type === PieceType.KING) {
      if (this.rules.variant === GameVariant.INTERNATIONAL) {
        ...
        // Standard Kings (move only 1 step)
        for (const dir of dirs) {
  ...
  private getValidJumpsForPiece(start: Position, piece: Piece, currentPos: Position = start, capturedSoFar: Position[] = []): Move[] {
    ...
    if (piece.type === PieceType.KING) {
      if (this.rules.variant === GameVariant.INTERNATIONAL) {
        ...
        // Standard King captures (only 1 step)
        for (const dir of dirs) {
      ...
      // Men captures
      // International: men can jump backwards. Standard: only forwards
