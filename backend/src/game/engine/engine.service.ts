...
export class DraughtsEngine {
  ...
  get boardSize(): number {
    return this.rules.variant === GameVariant.INTERNATIONAL ? 10 : 8;
  }

  get forceMajorityCapture(): boolean {
    return this.rules.variant === GameVariant.INTERNATIONAL;
  }
  ...
  private getValidNormalMovesForPiece(pos: Position, piece: Piece): Move[] {
    ...
    if (piece.type === PieceType.KING) {
      if (this.rules.variant === GameVariant.INTERNATIONAL) {
        ...
        // STANDARD kings only move one step
        for (const dir of dirs) {
  ...
  private getValidJumpsForPiece(start: Position, piece: Piece, currentPos: Position = start, capturedSoFar: Position[] = []): Move[] {
    ...
    if (piece.type === PieceType.KING) {
      if (this.rules.variant === GameVariant.INTERNATIONAL) {
        ...
        // STANDARD King captures (jump only adjacent, land immediately behind)
        for (const dir of dirs) {
      ...
      // Men captures
      // In STANDARD, men only jump forward. In INTERNATIONAL, men can jump backward.
      const captureDirs = this.rules.variant === GameVariant.INTERNATIONAL ? dirs : this.getMoveDirections(piece);
      ...
      for (const dir of captureDirs) {
        ...
        if (overPiece && overPiece.color !== piece.color && landPos === null && !alreadyCaptured) {
          ...
          // Check for promotion mid-jump (STANDARD rules: promotion stops the turn)
          let promoted = false;
          if (this.rules.variant === GameVariant.STANDARD) {
             if (piece.color === PieceColor.LIGHT && landR === 0) {
                 promoted = true;
             } else if (piece.color === PieceColor.DARK && landR === this.boardSize - 1) {
                 promoted = true;
             }
          }

          if (promoted) {
              // If promoted in STANDARD, stop the jump sequence here
              jumps.push({ from: start, to: { row: landR, col: landC }, captured: newCaptured });
          } else {
              const originalCurrent = this.board[currentPos.row][currentPos.col];
              this.board[currentPos.row][currentPos.col] = null;
  ...
  public makeMove(move: Move): boolean {
    ...
    if (piece.type === PieceType.MAN) {
      ...
      } else if (piece.color === PieceColor.DARK && move.to.row === this.boardSize - 1) {
        piece.type = PieceType.KING;
      }
    }
