...
export class DraughtsEngine {
  ...
  private getValidNormalMovesForPiece(pos: Position, piece: Piece): Move[] {
    ...
    const isInternational = this.rules.boardSize === 10;
    ...
    if (piece.type === PieceType.KING) {
      if (isInternational) {
        ...
        // Standard 8x8 King (moves 1 step backward or forward)
        for (const dir of dirs) {
  ...
  private getValidJumpsForPiece(start: Position, piece: Piece, currentPos: Position = start, capturedSoFar: Position[] = []): Move[] {
    ...
    if (piece.type === PieceType.KING) {
      if (isInternational) {
        ...
        // Standard 8x8 King captures (short range jumps)
        for (const dir of dirs) {
      ...
      for (const dir of dirs) {
        ...
        if (overPiece && overPiece.color !== piece.color && landPos === null && !alreadyCaptured) {
          ...
          // In 8x8 Standard, promotion to king immediately ends the turn even in multi-jump sequence
          const isPromotion = !isInternational && piece.type === PieceType.MAN &&
