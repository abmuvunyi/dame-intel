...
export class DraughtsEngine {
  ...
  private getValidJumpsForPiece(start: Position, piece: Piece, currentPos: Position = start, capturedSoFar: Position[] = []): Move[] {
    ...
    if (piece.type === PieceType.KING) {
      ...
      // Men captures
      // In 10x10, men can capture backwards (all 4 directions). In 8x8, only forwards.
      ...
      for (const dir of captureDirs) {
        ...
        if (overPiece && overPiece.color !== piece.color && landPos === null && !alreadyCaptured) {
          ...
          // Check for immediate promotion in 8x8 rules during a multi-jump
          let endsTurnDueToPromotion = false;
          if (this.rules.boardSize === 8) {
             if (piece.color === PieceColor.LIGHT && landR === 0) endsTurnDueToPromotion = true;
             if (piece.color === PieceColor.DARK && landR === this.rules.boardSize - 1) endsTurnDueToPromotion = true;
          }
