  ...
  private getValidNormalMovesForPiece(pos: Position, piece: Piece): Move[] {
    ...
    if (piece.type === PieceType.KING) {
      if (this.rules.boardSize === 10) {
        ...
        // 8x8 Kings move one step diagonally
        for (const dir of dirs) {
  ...
  private getValidJumpsForPiece(start: Position, piece: Piece, currentPos: Position = start, capturedSoFar: Position[] = []): Move[] {
    ...
    if (piece.type === PieceType.KING) {
      if (this.rules.boardSize === 10) {
        ...
        // 8x8 King captures (short range, one step over)
        for (const dir of dirs) {
      ...
      // Men captures
      // In 8x8, men can only capture forwards. In 10x10, they can capture backwards.
      ...
      for (const dir of captureDirs) {
        ...
        if (overPiece && overPiece.color !== piece.color && landPos === null && !alreadyCaptured) {
          ...
          // In 8x8 standard draughts, men immediately end their turn upon promotion to a king during a multi-jump sequence.
          const isPromotion = (piece.color === PieceColor.LIGHT && landR === 0) || (piece.color === PieceColor.DARK && landR === this.rules.boardSize - 1);
          let subJumps: Move[] = [];
          if (this.rules.boardSize === 8 && piece.type === PieceType.MAN && isPromotion) {
