...
export class DraughtsEngine {
  ...
  private getValidNormalMovesForPiece(pos: Position, piece: Piece): Move[] {
    ...
    if (piece.type === PieceType.KING) {
      if (this.rules.boardSize === 10) {
        // International Kings can fly (slide across empty diagonals)
  ...
  private getValidJumpsForPiece(start: Position, piece: Piece, currentPos: Position = start, capturedSoFar: Position[] = []): Move[] {
    ...
    if (piece.type === PieceType.KING) {
      if (this.rules.boardSize === 10) {
        ...
        // Standard 8x8 King captures (moves 1 step, captures adjacent)
        for (const dir of allDirs) {
          ...
          if (overPiece && overPiece.color !== piece.color && landPos === null && !alreadyCaptured) {
             ...
             // In 8x8, pieces are captured immediately (can't cross same square twice in a way that matters since captures are immediate, but we track to prevent loops)
             const originalCurrent = this.board[currentPos.row][currentPos.col];
             this.board[currentPos.row][currentPos.col] = null;
             this.board[landR][landC] = piece;

             // Remove piece temporarily during jump to allow jumping over the space it occupied (in 8x8 rules, some variations say removed immediately, some say at end. We will assume removed at end but not jumpable twice).
             // Actually standard 8x8 removes at end of turn.
             const subJumps = this.getValidJumpsForPiece(start, piece, { row: landR, col: landC }, newCaptured);
      ...
      // Men captures
      // In 10x10, men can capture backward. In 8x8, men capture forward only.
      let captureDirs = allDirs;
      if (this.rules.boardSize === 8) {
          const forward = piece.color === PieceColor.LIGHT ? -1 : 1;
          captureDirs = [{ dr: forward, dc: -1 }, { dr: forward, dc: 1 }];
      }
      ...
      for (const dir of captureDirs) {
        ...
        if (overPiece && overPiece.color !== piece.color && landPos === null && !alreadyCaptured) {
          ...
          // Memory Rule: In 8x8 standard draughts, men immediately end their turn upon promotion to a king during a multi-jump sequence.
          let promoteEnd = false;
          if (this.rules.boardSize === 8 && piece.type === PieceType.MAN) {
             if (piece.color === PieceColor.LIGHT && landR === 0) promoteEnd = true;
             else if (piece.color === PieceColor.DARK && landR === 7) promoteEnd = true;
          }
