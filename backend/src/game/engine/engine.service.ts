...
export class DraughtsEngine {
  ...
  private getValidJumpsForPiece(start: Position, piece: Piece, currentPos: Position = start, capturedSoFar: Position[] = []): Move[] {
    ...
    if (piece.type === PieceType.KING) {
      ...
      // Men captures
      // In 8x8, men can only capture forwards. In 10x10, men can capture backwards (all directions).
      const menDirs = this.rules.boardSize === 10 ? dirs : this.getMoveDirections(piece);
      ...
      for (const dir of menDirs) {
        ...
        if (overPiece && overPiece.color !== piece.color && landPos === null && !alreadyCaptured) {
          ...
          // 8x8 promotion rule: turn ends immediately if promoting during a sequence
          const willPromote = (piece.color === PieceColor.LIGHT && landR === 0) ||
                              (piece.color === PieceColor.DARK && landR === this.rules.boardSize - 1);
          const stopBecausePromoted = this.rules.boardSize === 8 && willPromote && piece.type === PieceType.MAN;
