...
export class DraughtsEngine {
  ...
  private getValidNormalMovesForPiece(pos: Position, piece: Piece): Move[] {
    ...
    if (piece.type === PieceType.KING) {
      if (this.rules.boardSize === 10) {
        ...
        // 8x8 Kings are short-range (move 1 step diagonally in all directions)
        for (const dir of dirs) {
  ...
  private getValidJumpsForPiece(start: Position, piece: Piece, currentPos: Position = start, capturedSoFar: Position[] = []): Move[] {
    ...
    // In 8x8, Men can only jump forwards. 10x10 Men can jump in all directions.
    const forward = piece.color === PieceColor.LIGHT ? -1 : 1;
    const dirs = piece.type === PieceType.MAN && this.rules.boardSize === 8
    ...
    if (piece.type === PieceType.KING) {
      if (this.rules.boardSize === 10) {
        ...
        // 8x8 Short-range King captures (must jump exactly 1 piece and land immediately behind it)
        for (const dir of dirs) {
      ...
      for (const dir of dirs) {
        ...
        if (overPiece && overPiece.color !== piece.color && landPos === null && !alreadyCaptured) {
          ...
          // In 8x8, if a man lands on the promotion row during a jump, its turn ends immediately.
          const isPromotion = this.rules.boardSize === 8 &&
