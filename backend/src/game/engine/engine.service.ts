...
export class DraughtsEngine {
  ...
  private getValidNormalMovesForPiece(pos: Position, piece: Piece): Move[] {
    ...
    if (piece.type === PieceType.KING) {
      if (this.rules.boardSize === 10) {
        ...
        // Short-range King moves 1 step diagonally
        for (const dir of dirs) {
  ...
  private getValidJumpsForPiece(start: Position, piece: Piece, currentPos: Position = start, capturedSoFar: Position[] = []): Move[] {
    ...
    if (piece.type === PieceType.KING) {
      ...
      // Men captures
      // For 8x8, men capture forward only. For 10x10, men can capture backward.
      ...
      for (const dir of captureDirs) {
        ...
        if (overPiece && overPiece.color !== piece.color && landPos === null && !alreadyCaptured) {
          ...
          // Check if piece promoted during this jump
          const promoted = (piece.color === PieceColor.LIGHT && landR === 0) || (piece.color === PieceColor.DARK && landR === this.rules.boardSize - 1);
          let subJumps: Move[] = [];

          if (promoted && this.rules.boardSize === 8) {
            // In 8x8, if a man reaches the king row during a multi-jump, the turn ends immediately.
            subJumps = [];
          } else {
