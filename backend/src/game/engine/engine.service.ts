...
export class DraughtsEngine {
  ...
  private getValidNormalMovesForPiece(pos: Position, piece: Piece): Move[] {
    ...
    if (piece.type === PieceType.KING) {
      if (this.rules.boardSize === 10) {
        ...
        // Short-range Kings (8x8) move one square in any diagonal direction
        for (const dir of dirs) {
  ...
  private getValidJumpsForPiece(start: Position, piece: Piece, currentPos: Position = start, capturedSoFar: Position[] = []): Move[] {
    ...
    if (piece.type === PieceType.KING) {
      if (this.rules.boardSize === 10) {
        ...
        for (const dir of dirs) {
          ...
          if (overPiece && overPiece.color !== piece.color && landPos === null && !alreadyCaptured) {
            ...
            // Short-range kings remove pieces immediately in standard rules, but our engine structure evaluates sub-jumps here
            const subJumps = this.getValidJumpsForPiece(start, piece, { row: landR, col: landC }, newCaptured);
      ...
      // Men captures
      for (const dir of dirs) {
        // 8x8 standard: Men can only capture forwards
        ...
        if (overPiece && overPiece.color !== piece.color && landPos === null && !alreadyCaptured) {
          ...
          // 8x8 Standard: immediate turn end on promotion
          const isPromotionRank = (piece.color === PieceColor.LIGHT && landR === 0) || (piece.color === PieceColor.DARK && landR === this.rules.boardSize - 1);
          const stopJumpForPromotion = this.rules.boardSize === 8 && isPromotionRank;
