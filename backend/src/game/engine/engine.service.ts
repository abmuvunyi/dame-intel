  ...
  private getValidNormalMovesForPiece(pos: Position, piece: Piece): Move[] {
    ...
    if (piece.type === PieceType.KING) {
      if (this.rules.boardSize === 10) {
        // Kings can fly in 10x10 (slide across empty diagonals)
        ...
        // Standard Kings in 8x8 move exactly 1 square
        for (const dir of dirs) {
    ...
    if (piece.type === PieceType.KING) {
      ...
      for (const dir of menDirs) {
        ...
        if (overPiece && overPiece.color !== piece.color && landPos === null && !alreadyCaptured) {
          ...
          // For 8x8, if men land on the promotion row during a jump, they immediately promote and their turn ends
          let isPromotion = false;
          if (this.rules.boardSize === 8) {
             if (piece.color === PieceColor.LIGHT && landR === 0) isPromotion = true;
             else if (piece.color === PieceColor.DARK && landR === 7) isPromotion = true;
          }
