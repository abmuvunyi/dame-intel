    ...
    if (piece.type === PieceType.KING) {
      if (this.rules.boardSize === 8) {
        ...
        for (const dir of dirs) {
          ...
          if (this.isValidPos(r, c) && this.board[r][c] === null) {
        ...
        for (const dir of dirs) {
          let step = 1;
          while (true) {
            const r = pos.row + dir.dr * step;
            const c = pos.col + dir.dc * step;
    ...
    if (piece.type === PieceType.KING) {
      if (this.rules.boardSize === 8) {
        ...
        for (const dir of dirs) {
          ...
          if (this.rules.boardSize === 8 && isPromotion) {
            // Turn ends immediately upon promotion in 8x8
          } else {
      ...
      // Men captures
      const manDirs = this.rules.boardSize === 8 ? this.getMoveDirections(piece) : dirs;
