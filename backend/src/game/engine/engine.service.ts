  ...
  private getValidJumpsForPiece(start: Position, piece: Piece, currentPos: Position = start, capturedSoFar: Position[] = []): Move[] {
    ...
    if (piece.type === PieceType.KING) {
      if (this.rules.boardSize === 10) {
        ...
        // 8x8 King captures: only jumps 1 square over
        for (const dir of dirs) {
      ...
      // Men captures
      // In 10x10 men can capture backwards. In 8x8 they capture only forwards.
      let moveDirs = dirs;
      if (this.rules.boardSize === 8) {
         const forward = piece.color === PieceColor.LIGHT ? -1 : 1;
         moveDirs = [{ dr: forward, dc: -1 }, { dr: forward, dc: 1 }];
      }
      ...
      for (const dir of moveDirs) {
        ...
        if (overPiece && overPiece.color !== piece.color && landPos === null && !alreadyCaptured) {
          ...
          // 8x8 check: if a man reaches the last rank during a jump, it becomes a king and the turn ends IMMEDIATELY.
          if (this.rules.boardSize === 8) {
            const isLastRank = (piece.color === PieceColor.LIGHT && landR === 0) || (piece.color === PieceColor.DARK && landR === this.rules.boardSize - 1);
            if (isLastRank) {
               jumps.push({ from: start, to: { row: landR, col: landC }, captured: newCaptured });
               continue; // Stop multi-jumping
            }
          }
