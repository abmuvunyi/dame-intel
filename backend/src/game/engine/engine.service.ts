  ...
  private getValidNormalMovesForPiece(pos: Position, piece: Piece): Move[] {
    ...
    if (piece.type === PieceType.KING) {
      if (this.rules.boardSize === 10) {
        // Flying kings in International Checkers
        ...
        // Standard 8x8 Checkers: short-range kings
        for (const dir of dirs) {
  ...
  private getValidJumpsForPiece(start: Position, piece: Piece, currentPos: Position = start, capturedSoFar: Position[] = []): Move[] {
    ...
    if (piece.type === PieceType.KING) {
      if (this.rules.boardSize === 10) {
        ...
        // Short-range King captures (Standard 8x8)
        for (const dir of dirsAll) {
      ...
      // Men captures
      // In 8x8, men can only capture forwards. In 10x10, men can capture forwards and backwards.
      const dirs = this.rules.boardSize === 8 ? this.getMoveDirections(piece) : dirsAll;
      ...
      for (const dir of dirs) {
        ...
        if (overPiece && overPiece.color !== piece.color && landPos === null && !alreadyCaptured) {
          ...
          // Promotion rule during multi-jump for 8x8
          const isPromotionRow = (piece.color === PieceColor.LIGHT && landR === 0) || (piece.color === PieceColor.DARK && landR === this.rules.boardSize - 1);
          let subJumps: Move[] = [];

          if (this.rules.boardSize === 8 && isPromotionRow) {
            // Turn ends immediately upon reaching king row in 8x8
            subJumps = [];
          } else {
            const originalCurrent = this.board[currentPos.row][currentPos.col];
            this.board[currentPos.row][currentPos.col] = null;
