...
export class DraughtsEngine {
  ...
  private getValidNormalMovesForPiece(pos: Position, piece: Piece): Move[] {
    ...
    if (piece.type === PieceType.KING) {
      if (this.rules.boardSize === 10) {
        ...
        // In 8x8, Kings move exactly 1 square diagonally like men but backwards too
  ...
  private getValidJumpsForPiece(start: Position, piece: Piece, currentPos: Position = start, capturedSoFar: Position[] = []): Move[] {
    ...
    if (piece.type === PieceType.KING) {
      ...
      // Men captures
      // In 10x10 men can jump backwards. In 8x8 men only jump forwards
      let captureDirs = dirs;
      if (this.rules.boardSize === 8) {
        captureDirs = this.getMoveDirections(piece); // Only forward
      }
      ...
      for (const dir of captureDirs) {
        ...
        if (overPiece && overPiece.color !== piece.color && landPos === null && !alreadyCaptured) {
          ...
          // Standard 8x8 rule: Man promotes immediately if landing on last row during multi-jump
          let subJumps: Move[] = [];
          if (this.rules.boardSize === 8 && piece.type === PieceType.MAN && (landR === 0 || landR === this.rules.boardSize - 1)) {
            // No sub-jumps, turn ends
          } else {
