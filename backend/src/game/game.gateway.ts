...
export class GameGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  handleJoinMatchmaking(@ConnectedSocket() client: Socket, @MessageBody() data?: { tournamentId?: number, variant?: GameVariant }) {
    ...
    // Wait list logic
    const variant = data?.variant || GameVariant.STANDARD;
    (client as any).waitingVariant = variant; // Store preference temporarily on socket for matchmaking

    // Look for a match
    let matchIdx = -1;
    for (let i = 0; i < this.waitingPlayers.length; i++) {
       const p = this.waitingPlayers[i];
       // Match if tournament ID matches (or both lack it) AND variant preference matches
       const pVariant = (this.server.sockets.sockets.get(p.socketId) as any)?.waitingVariant || GameVariant.STANDARD;
       if (p.socketId !== client.id && p.tournamentId === data?.tournamentId && pVariant === variant) {
          matchIdx = i;
          break;
       }
    }
