...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-transparent flex flex-col items-center py-10 px-4">
        ...
        <div className="flex space-x-4">
          {isAuthenticated ? (
            <>
              ...
              <button onClick={handleLogout} className="px-4 py-2 text-sm font-medium text-slate-700 bg-[#2b2926] border border-slate-300 rounded hover:bg-slate-50 transition shadow-sm">
                Log Out
              </button>
            </>
          ) : (
      ...
      <div className="w-full max-w-5xl bg-[#2b2926] rounded-xl shadow-2xl overflow-hidden border border-gray-100">
        <div className="p-8">
          <GameBoard />
        </div>
      </div>
