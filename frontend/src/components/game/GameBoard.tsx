        ...
        <div className="flex flex-col space-y-4 pt-4 border-t border-gray-200 w-64">
          <div className="flex justify-center space-x-2 mb-2">
            <button
              ...
              className={`px-3 py-1 rounded text-sm font-semibold transition ${selectedVariant === GameVariant.STANDARD ? 'bg-indigo-600 text-white shadow' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
            >
              8x8 Standard
            </button>
            <button
              onClick={() => setSelectedVariant(GameVariant.INTERNATIONAL)}
              className={`px-3 py-1 rounded text-sm font-semibold transition ${selectedVariant === GameVariant.INTERNATIONAL ? 'bg-indigo-600 text-white shadow' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
            >
              10x10 Int'l
            </button>
          </div>
          ...
          {board.map((row, r) => (
            <div key={r} className="flex">
              {row.map((cell, c) => {
                ...
                const isStandard = activeVariant === GameVariant.STANDARD;
                ...
                return (
                  <div
                    ...
                    className={`
                      ${isStandard ? 'w-10 h-10 sm:w-16 sm:h-16' : 'w-8 h-8 sm:w-12 sm:h-12 md:w-14 md:h-14'}
                      flex items-center justify-center ${squareBg} cursor-pointer transition-colors duration-150
                    `}
                  >
                    {cell && (
                      <div className={`
                        ${isStandard ? 'w-8 h-8 sm:w-12 sm:h-12' : 'w-6 h-6 sm:w-10 sm:h-10 md:w-12 md:h-12'}
                        rounded-full shadow-md flex items-center justify-center text-white font-bold transform transition-transform hover:scale-105
