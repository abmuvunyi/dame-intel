...
export default function GameBoard() {
  ...
  // Auto-update forceMajorityCapture when boardSize changes
  useEffect(() => {
    if (boardSize === 8) {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-screen space-y-4">
        ...
        <div className="flex flex-col space-y-4 pt-4 border-t border-[#3c3a38] w-64">
          ...
          <div className="bg-[#262522] p-4 rounded-lg shadow-inner flex flex-col space-y-3">
            ...
            <label className="text-sm flex justify-between items-center text-gray-400">
               ...
               <select
                  ...
                  className="ml-2 border border-[#4c4a48] rounded p-1 text-sm bg-[#302e2b] text-white"
               >
            ...
            <label className="text-sm flex items-center gap-2 text-gray-400 cursor-pointer">
               <input
                  ...
                  className="rounded bg-[#302e2b] border-[#4c4a48]"
               />
          ...
          <button
            ...
            className="w-full px-6 py-3 bg-[#81b64c] text-white font-bold rounded shadow hover:bg-[#8fd053] transition"
          >
        ...
        {activeGames.length > 0 && (
          <div className="mt-8 w-full">
            <h3 className="text-xl font-bold mb-4 text-center text-gray-300">Live Games</h3>
            <ul className="space-y-2">
              {activeGames.map((game, i) => (
                <li key={i} className="flex justify-between items-center bg-[#262522] p-3 rounded border border-[#3c3a38]">
                   <span className="font-medium text-gray-300">{game.player1} vs {game.player2}</span>
                   <button
                     onClick={() => handleWatchGame(game.roomId)}
                     className="px-4 py-1 bg-[#4c4a48] text-white rounded text-sm hover:bg-[#5c5a58]"
                   >
  ...
  return (
    <div className="flex flex-col md:flex-row justify-center py-10 gap-8 max-w-6xl mx-auto px-4">
      ...
      <div className="flex flex-col items-center space-y-4">
        ...
        {myColor && !status.includes('Game Over') && (
          <div className="flex gap-4">
            <button onClick={handleOfferDraw} className="px-4 py-2 bg-[#4c4a48] text-white rounded shadow hover:bg-[#5c5a58] text-sm font-semibold transition">
              Offer Draw
            </button>
            <button onClick={handleResign} className="px-4 py-2 bg-red-800 text-white rounded shadow hover:bg-red-900 text-sm font-semibold transition">
              Resign
            </button>
          </div>
        )}
        ...
        <div className="border-[6px] border-[#302e2b] p-1 bg-[#262522] shadow-2xl rounded-sm">
          {board.map((row, r) => (
            <div key={r} className="flex">
              {row.map((cell, c) => {
                ...
                let squareBg = isDarkSquare ? 'bg-[#769656]' : 'bg-[#eeeed2]'; // Chess.com colors
                if (isSelected) squareBg = 'bg-[#baca44]'; // Selected highlight color
                if (isHighlighted) squareBg = 'bg-[#f4f680] opacity-80'; // Destination highlight
                ...
                const pieceClass = is10x10 ? 'w-8 h-8 sm:w-10 sm:h-10 border-[1px]' : 'w-10 h-10 sm:w-12 sm:h-12 border-[2px]';
                const stackClass = is10x10 ? 'w-8 h-8 sm:w-10 sm:h-10 border-[1px] absolute -top-1 -left-1' : 'w-10 h-10 sm:w-12 sm:h-12 border-[2px] absolute -top-1.5 -left-1.5';
                  ...
                  >
                    {cell && (
                      <div className={`
                        ...
                        ${cell.color === PieceColor.LIGHT ? 'bg-[#f8f8f8] border-[#e8e8e8]' : 'bg-[#565352] border-[#403d39]'}
                        ${cell.type === PieceType.KING ? kingOffset : ''}
                      `}>
                        {/* Stacked piece visual for King */}
                        {cell.type === PieceType.KING && (
                          <div className={`
                            ${stackClass} rounded-full shadow-[0_2px_4px_rgba(0,0,0,0.4)]
                            ${cell.color === PieceColor.LIGHT ? 'bg-[#f8f8f8] border-[#e8e8e8]' : 'bg-[#565352] border-[#403d39]'}
                          `} />
                        )}
                      </div>
      ...
      {/* Chat Column */}
      <div className="w-full md:w-80 flex flex-col bg-[#262522] rounded-lg shadow-xl border border-[#3c3a38] h-[600px]">
          ...
          {chatMessages.length === 0 ? (
            ...
            chatMessages.map((msg, i) => (
              <div key={i} className="flex flex-col">
                ...
                <span className="bg-[#302e2b] text-white p-2 rounded text-sm inline-block w-fit max-w-[90%] break-words">
                  {msg.message}
                </span>
              </div>
            ))
        ...
        <div className="p-3 border-t border-[#3c3a38] bg-[#302e2b] rounded-b-lg">
          <form onSubmit={handleSendMessage} className="flex gap-2">
            <input
              ...
              className="flex-1 text-sm border border-[#4c4a48] bg-[#262522] text-white rounded px-3 py-2 focus:outline-none focus:border-[#81b64c]"
            />
            <button
              type="submit"
              className="bg-[#81b64c] text-white px-4 py-2 rounded text-sm font-semibold hover:bg-[#8fd053] transition"
            >
              Send
            </button>
          </form>
