        ...
        <div className="flex flex-col space-y-4 pt-4 border-t border-gray-200 w-64">
          <div className="flex flex-col space-y-2 mb-2">
            ...
            <select
              ...
              className="px-3 py-2 border rounded shadow-sm text-gray-800"
              disabled={!!tournamentIdToJoin}
            >
          ...
          {board.map((row, r) => (
            <div key={r} className="flex">
              {row.map((cell, c) => {
                ...
                const isTenByTen = board.length === 10;
                const squareSizeClass = isTenByTen ? 'w-10 h-10 sm:w-12 sm:h-12' : 'w-14 h-14 sm:w-16 sm:h-16';
                const pieceSizeClass = isTenByTen ? 'w-7 h-7 sm:w-9 sm:h-9 border-[3px]' : 'w-10 h-10 sm:w-12 sm:h-12 border-4';
