...
export default function GameBoard() {
  ...
  if (!board) {
    return (
      <div className="flex flex-col items-center justify-center h-screen space-y-4">
        ...
        <div className="flex flex-col space-y-4 pt-4 border-t border-[#3d3b39] w-64">
          ...
          <div className="bg-[#262421] p-4 rounded-lg shadow-inner flex flex-col space-y-3">
            ...
            <label className="text-sm flex justify-between items-center text-gray-400">
               ...
               <select
                  ...
                  className="ml-2 border rounded p-1 text-sm bg-[#3d3b39] text-white border-gray-600"
               >
          ...
          <button
            ...
            className="w-full px-6 py-3 bg-[#81b64c] text-white font-semibold rounded shadow hover:bg-[#a3d160] transition"
          >
          ...
          <div className="grid grid-cols-2 gap-2">
            {[1, 2, 3, 4, 5, 6, 7].map(level => (
              <button
                ...
                className={`w-full px-2 py-2 text-white rounded transition text-sm ${level > 4 ? 'bg-red-800 hover:bg-red-900 col-span-2' : 'bg-[#3d3b39] hover:bg-[#4d4b49] text-gray-300'}`}
              >
          ...
          <div className="mt-8 w-full">
            ...
            <ul className="space-y-2">
              {activeGames.map((game, i) => (
                <li key={i} className="flex justify-between items-center bg-[#262421] p-3 rounded border border-[#3d3b39]">
                   <span className="font-medium text-gray-300">{game.player1} vs {game.player2}</span>
                   <button
                     onClick={() => handleWatchGame(game.roomId)}
                     className="px-4 py-1 bg-[#81b64c] text-white rounded text-sm hover:bg-[#a3d160]"
                   >
  ...
  return (
    <div className="flex flex-col md:flex-row justify-center py-10 gap-8 max-w-6xl mx-auto px-4">
          ...
          {board.map((row, r) => (
            <div key={r} className="flex">
              {row.map((cell, c) => {
                ...
                let squareBg = isDarkSquare ? 'bg-[#769656]' : 'bg-[#eeeed2]'; // Chess.com style board colors
                ...
                return (
                  ...
                  >
                    {cell && (
                      <div className={`
                        ${pieceClass} rounded-full shadow-md flex items-center justify-center text-white font-bold transform transition-transform hover:scale-105 relative
                        ${cell.color === PieceColor.LIGHT ? 'bg-slate-100 border-slate-300' : 'bg-slate-800 border-slate-900'}
                      `}>
                        {cell.type === PieceType.KING && (
                          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-3/4 h-3/4 text-yellow-400 drop-shadow-[0_2px_2px_rgba(0,0,0,0.8)]"><path d="M12 2l3 7h7l-5.5 4.5 2 7.5-6.5-5-6.5 5 2-7.5-5.5-4.5h7z" /></svg>
                        )}
                      </div>
                    )}
                  </div>
      ...
      {/* Chat Column */}
      <div className="w-full md:w-80 flex flex-col bg-[#262421] rounded-lg shadow-xl border border-[#3d3b39] h-[600px]">
        <div className="bg-[#1a1918] text-gray-300 p-4 rounded-t-lg">
          ...
          {chatMessages.length === 0 ? (
            ...
            chatMessages.map((msg, i) => (
              <div key={i} className="flex flex-col">
                ...
                <span className="bg-[#3d3b39] p-2 rounded shadow-sm text-sm border border-[#4d4b49] text-gray-200 inline-block w-fit max-w-[90%] break-words">
                  {msg.message}
                </span>
              </div>
            ))
        ...
        <div className="p-3 border-t border-[#3d3b39] bg-[#262421] rounded-b-lg">
          <form onSubmit={handleSendMessage} className="flex gap-2">
            <input
              ...
              className="flex-1 text-sm bg-[#3d3b39] text-white border border-[#4d4b49] rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#81b64c]"
            />
            <button
              type="submit"
              className="bg-[#81b64c] text-white px-4 py-2 rounded text-sm font-semibold hover:bg-[#a3d160] transition"
            >
              Send
            </button>
          </form>
