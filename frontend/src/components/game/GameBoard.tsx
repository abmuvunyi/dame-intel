...
export default function GameBoard() {
  ...
  const [variant, setVariant] = useState<string>('STANDARD_8X8');
      ...
      <div className="flex flex-col items-center justify-center h-screen space-y-4">
        ...
        <div className="flex flex-col space-y-4 pt-4 border-t border-gray-200 w-64">
          ...
          <div className="flex flex-col mb-2">
            ...
            >
              <option value="STANDARD_8X8">Standard (8x8)</option>
              <option value="INTERNATIONAL_10X10">International (10x10)</option>
            </select>
          </div>
          ...
          {board.map((row, r) => (
            <div key={r} className="flex">
              {row.map((cell, c) => {
                ...
                // Adjust square size based on the board size so it fits neatly
                const is10x10 = board.length === 10;
                const sqSizeClasses = is10x10 ? 'w-10 h-10 sm:w-12 sm:h-12' : 'w-14 h-14 sm:w-16 sm:h-16';
                const pieceSizeClasses = is10x10 ? 'w-8 h-8 sm:w-10 sm:h-10 text-xs' : 'w-10 h-10 sm:w-12 sm:h-12 text-sm';

                return (
                  <div
                    key={`${r}-${c}`}
                    onClick={() => handleSquareClick(r, c)}
                    className={`${sqSizeClasses} flex items-center justify-center ${squareBg} cursor-pointer transition-colors duration-150`}
                  >
                    {cell && (
                      <div className={`
                        ${pieceSizeClasses} rounded-full shadow-md flex items-center justify-center font-bold transform transition-transform hover:scale-105
                        ${cell.color === PieceColor.LIGHT ? 'bg-slate-100 border-[3px] sm:border-4 border-slate-300 text-slate-800' : 'bg-slate-800 border-[3px] sm:border-4 border-slate-900 text-slate-200'}
