  ...
  if (!board) {
    return (
      <div className="flex flex-col items-center justify-center space-y-4">
        <h1 className="text-3xl font-bold text-gray-200">Play Draughts</h1>
        <p className="text-gray-400">{status}</p>

        <div className="flex flex-col space-y-4 pt-4 border-t border-[#3c3a37] w-64">

          <div className="bg-[#262421] p-4 rounded-lg shadow-inner flex flex-col space-y-3">
            <h4 className="text-sm font-bold text-gray-300">Game Rules</h4>
            <label className="text-sm flex flex-col gap-1 text-gray-400">
               Board Size:
               <select
                  value={boardSize}
                  onChange={e => setBoardSize(parseInt(e.target.value))}
                  className="w-full border border-[#3c3a37] rounded p-2 text-sm bg-[#302e2b] text-white"
               >
          ...
          <button
            ...
            className="w-full px-6 py-4 bg-[#81b64c] text-white font-bold rounded-lg text-lg shadow-[0_5px_0_#537a2f] hover:bg-[#8cc254] transition transform active:translate-y-1 active:shadow-[0_2px_0_#537a2f]"
          >
            {tournamentIdToJoin ? 'Find Tournament Match' : 'Play Multiplayer'}
          </button>

          <div className="text-center pt-2 text-sm text-gray-500 font-bold uppercase tracking-widest">OR</div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {[1, 2, 3, 4, 5, 6, 7].map(level => (
              <button
                ...
                className={`w-full px-2 py-3 text-white font-bold rounded-md transition text-sm shadow-[0_4px_0_rgba(0,0,0,0.3)] hover:brightness-110 active:translate-y-1 active:shadow-[0_1px_0_rgba(0,0,0,0.3)] ${level > 4 ? 'bg-red-800 col-span-2' : 'bg-[#3c3a37]'}`}
              >
        ...
        {activeGames.length > 0 && (
          <div className="mt-8 w-full max-w-md bg-[#262421] p-4 rounded-lg">
            <h3 className="text-xl font-bold mb-4 text-center text-gray-300 border-b border-[#3c3a37] pb-2">Live Games</h3>
              ...
              {activeGames.map((game, i) => (
                <li key={i} className="flex justify-between items-center bg-[#302e2b] p-3 rounded border border-[#3c3a37]">
                   ...
                   <button
                     ...
                     className="px-4 py-1 bg-[#81b64c] text-white font-bold rounded text-sm hover:bg-[#8cc254]"
                   >
  ...
  return (
    <div className="flex flex-col md:flex-row justify-center py-10 gap-8 max-w-6xl mx-auto px-4">
      ...
      <div className="flex flex-col items-center space-y-4">
        <h1 className="text-2xl font-bold text-gray-200">Game Room</h1>
        ...
        {myColor && !status.includes('Game Over') && (
          <div className="flex gap-4">
            <button onClick={handleOfferDraw} className="px-4 py-2 bg-[#3c3a37] text-gray-200 rounded shadow hover:bg-gray-600 text-sm font-semibold transition">
              Offer Draw
            </button>
            <button onClick={handleResign} className="px-4 py-2 bg-red-900 text-red-100 rounded shadow hover:bg-red-800 text-sm font-semibold transition">
              Resign
            </button>
          </div>
        )}
        ...
        {drawOfferPending && (
          <div className="bg-yellow-900 border border-yellow-700 text-yellow-100 px-4 py-3 rounded relative shadow-md mt-2">
            <p className="font-bold">Draw Offered</p>
            <p className="text-sm">Your opponent has offered a draw.</p>
            <div className="mt-2 flex gap-2">
              <button onClick={handleAcceptDraw} className="bg-yellow-600 hover:bg-yellow-500 text-white font-bold py-1 px-3 rounded text-sm">Accept</button>
              <button onClick={handleDeclineDraw} className="bg-[#302e2b] hover:bg-gray-700 text-gray-200 font-semibold py-1 px-3 border border-gray-600 rounded shadow text-sm">Decline</button>
            </div>
          </div>
        )}

        <div className="border-[6px] border-[#302e2b] p-1 bg-[#262421] shadow-[0_10px_30px_rgba(0,0,0,0.5)] rounded-sm">
          {board.map((row, r) => (
            <div key={r} className="flex">
              {row.map((cell, c) => {
                ...
                if (isSelected) squareBg = isDarkSquare ? 'bg-[#baca44]' : 'bg-[#f4f680]'; // Highlighted color matching chess.com
                if (isHighlighted) squareBg = 'bg-[#ff6e59] opacity-80'; // Move hint
                ...
                return (
                  ...
                  >
                    {cell && (
                      <div className={`
                        ${pieceClass} rounded-full shadow-[0_2px_4px_rgba(0,0,0,0.4)] flex items-center justify-center text-white font-bold transform transition-transform hover:scale-105
                        ${cell.color === PieceColor.LIGHT ? 'bg-[#f9f9f9] border-[#e0e0e0]' : 'bg-[#403e3c] border-[#202020]'}
                      `}>
                        {/* High-quality King SVG Icon */}
                        {cell.type === PieceType.KING && (
                          <svg
                             xmlns="http://www.w3.org/2000/svg"
                             viewBox="0 0 512 512"
                             className={`w-3/4 h-3/4 drop-shadow-[0_1px_1px_rgba(0,0,0,0.5)] ${cell.color === PieceColor.LIGHT ? 'fill-[#403e3c]' : 'fill-[#f9f9f9]'}`}
                          >
                             <path d="M439.4 126.9l-61.9 148.9L256 128l-121.5 147.8L72.6 126.9c-8.9-21.3-39.7-16-41.9 6.8L12.4 430.4C10.7 447.8 24.3 464 41.8 464H470.2c17.5 0 31.1-16.2 29.4-33.6L481.3 133.7c-2.2-22.8-33-28.1-41.9-6.8zM256 104a32 32 0 1 0 0-64 32 32 0 1 0 0 64zm-144 0a32 32 0 1 0 0-64 32 32 0 1 0 0 64zm288 0a32 32 0 1 0 0-64 32 32 0 1 0 0 64z"/>
                          </svg>
                        )}
                      </div>
                    )}
      ...
      {/* Chat Column */}
      <div className="w-full md:w-80 flex flex-col bg-[#262421] rounded-lg shadow-xl border border-[#3c3a37] h-[600px]">
        <div className="bg-[#3c3a37] text-white p-4 rounded-t-lg">
          ...
          {chatMessages.length === 0 ? (
            ...
            chatMessages.map((msg, i) => (
              <div key={i} className="flex flex-col">
                ...
                <span className="bg-[#3c3a37] p-2 rounded shadow-sm text-sm text-gray-200 border border-[#4c4a47] inline-block w-fit max-w-[90%] break-words">
                  {msg.message}
                </span>
              </div>
            ))
        ...
        <div className="p-3 border-t border-[#3c3a37] bg-[#262421] rounded-b-lg">
          <form onSubmit={handleSendMessage} className="flex gap-2">
            <input
              ...
              className="flex-1 text-sm bg-[#302e2b] text-white border border-[#4c4a47] rounded px-3 py-2 focus:outline-none focus:ring-1 focus:ring-[#81b64c]"
            />
            <button
              type="submit"
              className="bg-[#81b64c] text-white px-4 py-2 rounded text-sm font-bold hover:bg-[#8cc254] transition"
            >
              Send
            </button>
          </form>
