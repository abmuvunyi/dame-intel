      ...
      <div className="flex flex-col items-center justify-center h-screen space-y-4">
        ...
        <div className="flex flex-col space-y-4 pt-4 border-t border-gray-200 w-64">
          <div className="flex flex-col space-y-2 mb-2">
            <label className="text-sm font-semibold text-gray-700">Select Game Variant:</label>
            <select
            ...
            <ul className="space-y-2">
              {activeGames.map((game, i) => (
                <li key={i} className="flex justify-between items-center bg-gray-50 p-3 rounded border">
                   <span className="font-medium text-gray-700">{game.player1} vs {game.player2} <span className="text-xs text-gray-500 ml-1">({game.variant === GameVariant.INTERNATIONAL ? '10x10' : '8x8'})</span></span>
                   <button
  ...
  return (
    <div className="flex flex-col md:flex-row justify-center py-10 gap-8 max-w-6xl mx-auto px-4">
      ...
      <div className="flex flex-col items-center space-y-4">
        ...
        <div className="border-[6px] border-slate-800 p-1 bg-slate-200 shadow-2xl rounded-sm inline-block">
          {board.map((row, r) => (
            <div key={r} className="flex">
              {row.map((cell, c) => {
                ...
                // Adjust square size based on board size (8x8 vs 10x10) so it fits on screen nicely
                const squareSizeClass = board.length === 10 ? 'w-10 h-10 sm:w-12 sm:h-12' : 'w-14 h-14 sm:w-16 sm:h-16';
                const pieceSizeClass = board.length === 10 ? 'w-8 h-8 sm:w-10 sm:h-10 text-xs sm:text-sm' : 'w-10 h-10 sm:w-12 sm:h-12 text-base';
                ...
                return (
                  <div
                    ...
                    className={`${squareSizeClass} flex-shrink-0 flex items-center justify-center ${squareBg} cursor-pointer transition-colors duration-150`}
                  >
                    {cell && (
                      <div className={`
                        ...
                        ${cell.color === PieceColor.LIGHT ? 'bg-slate-100 border-4 border-slate-300 text-slate-800' : 'bg-slate-800 border-[3px] sm:border-4 border-slate-900 text-slate-200'}
                        ${cell.type === PieceType.KING ? 'ring-[3px] sm:ring-4 ring-yellow-400' : ''}
                      `}>
