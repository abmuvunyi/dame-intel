  ...
  if (!board) {
    return (
      <div className="flex flex-col items-center justify-center space-y-4">
        ...
        <div className="flex flex-col space-y-4 pt-4 border-t border-[#3c3a38] w-64">
          ...
          <div className="bg-[#302e2b] p-4 rounded-lg shadow-inner flex flex-col space-y-3">
            ...
            <label className="text-sm flex justify-between items-center text-gray-400">
               ...
               <select
                  ...
                  className="ml-2 border border-[#3c3a38] rounded p-1 text-sm bg-[#262421] text-white"
               >
            ...
            <label className="text-sm flex items-center gap-2 text-gray-400 cursor-pointer">
               <input
                  ...
                  className="rounded bg-[#262421] border-[#3c3a38]"
               />
          ...
          <button
            ...
            className="w-full px-6 py-3 bg-[#81b64c] text-white font-bold rounded shadow hover:bg-[#8cc453] transition"
          >
          ...
          <div className="grid grid-cols-2 gap-2">
            {[1, 2, 3, 4, 5, 6, 7].map(level => (
              <button
                ...
                className={`w-full px-2 py-2 text-white font-bold rounded transition text-sm ${level > 4 ? 'bg-[#b33430] hover:bg-[#c23d3a] col-span-2' : 'bg-[#3c3a38] hover:bg-[#4a4744]'}`}
              >
          ...
          <div className="mt-8 w-full">
            ...
            <ul className="space-y-2">
              {activeGames.map((game, i) => (
                <li key={i} className="flex justify-between items-center bg-[#302e2b] p-3 rounded border border-[#3c3a38]">
                   <span className="font-medium text-gray-300">{game.player1} vs {game.player2}</span>
                   <button
                     onClick={() => handleWatchGame(game.roomId)}
                     className="px-4 py-1 bg-[#81b64c] text-white font-bold rounded text-sm hover:bg-[#8cc453]"
                   >
  ...
  return (
    <div className="flex flex-col md:flex-row justify-center py-10 gap-8 max-w-6xl mx-auto px-4">
      ...
      <div className="flex flex-col items-center space-y-4">
        ...
        <p className="text-xl font-bold text-[#81b64c]">
        ...
        {myColor && !status.includes('Game Over') && (
          <div className="flex gap-4">
            <button onClick={handleOfferDraw} className="px-4 py-2 bg-[#3c3a38] text-white font-bold rounded shadow hover:bg-[#4a4744] text-sm transition">
              Offer Draw
            </button>
            <button onClick={handleResign} className="px-4 py-2 bg-[#b33430] text-white font-bold rounded shadow hover:bg-[#c23d3a] text-sm transition">
              Resign
            </button>
          </div>
        )}
        ...
        {drawOfferPending && (
          <div className="bg-[#302e2b] border border-[#3c3a38] text-white px-4 py-3 rounded relative shadow-md mt-2">
            <p className="font-bold text-yellow-500">Draw Offered</p>
            <p className="text-sm text-gray-300">Your opponent has offered a draw.</p>
            <div className="mt-2 flex gap-2">
              <button onClick={handleAcceptDraw} className="bg-[#81b64c] hover:bg-[#8cc453] text-white font-bold py-1 px-3 rounded text-sm">Accept</button>
              <button onClick={handleDeclineDraw} className="bg-[#3c3a38] hover:bg-[#4a4744] text-white font-bold py-1 px-3 border border-[#4a4744] rounded shadow text-sm">Decline</button>
            </div>
          </div>
        )}
          ...
          {board.map((row, r) => (
            <div key={r} className="flex">
              {row.map((cell, c) => {
                ...
                let squareBg = isDarkSquare ? 'bg-[#769656]' : 'bg-[#eeeed2]'; // Chess.com aesthetic colors
                if (isSelected) squareBg = 'bg-[#baca44]'; // Selected highlight
                if (isHighlighted) squareBg = 'bg-[#f6f669] opacity-90'; // Highlight destination
      ...
      {/* Chat Column */}
      <div className="w-full md:w-80 flex flex-col bg-[#262421] rounded-lg shadow-xl border border-[#3c3a38] h-[600px]">
        <div className="bg-[#302e2b] text-white p-4 rounded-t-lg border-b border-[#3c3a38]">
          ...
          {chatMessages.length === 0 ? (
            ...
            chatMessages.map((msg, i) => (
              <div key={i} className="flex flex-col">
                ...
                <span className="bg-[#3c3a38] p-2 rounded shadow-sm text-sm text-white inline-block w-fit max-w-[90%] break-words">
                  {msg.message}
                </span>
              </div>
            ))
        ...
        <div className="p-3 border-t border-[#3c3a38] bg-[#302e2b] rounded-b-lg">
          <form onSubmit={handleSendMessage} className="flex gap-2">
            <input
              ...
              className="flex-1 text-sm bg-[#262421] border border-[#3c3a38] text-white rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#81b64c]"
            />
            <button
              type="submit"
              className="bg-[#81b64c] text-white px-4 py-2 rounded text-sm font-bold hover:bg-[#8cc453] transition"
            >
              Send
            </button>
          </form>
