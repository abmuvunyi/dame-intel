        ...
        <div className="flex flex-col space-y-4 pt-4 border-t border-gray-200 w-64">
          <div className="flex justify-center space-x-2 mb-2">
            <button
              ...
              className={`px-4 py-2 text-sm rounded ${selectedVariant === GameVariant.STANDARD ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}
            >
              8x8 Standard
            </button>
            <button
              onClick={() => setSelectedVariant(GameVariant.INTERNATIONAL)}
              className={`px-4 py-2 text-sm rounded ${selectedVariant === GameVariant.INTERNATIONAL ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}
            >
              10x10 International
            </button>
          </div>
          ...
          {board.map((row, r) => (
            <div key={r} className="flex">
              {row.map((cell, c) => {
                ...
                // Adjust size based on board length to prevent overflow for 10x10 boards
                const boardSize = board.length;
                const squareSizeClass = boardSize === 10 ? 'w-10 h-10 sm:w-12 sm:h-12' : 'w-14 h-14 sm:w-16 sm:h-16';
                const pieceSizeClass = boardSize === 10 ? 'w-8 h-8 sm:w-10 sm:h-10' : 'w-10 h-10 sm:w-12 sm:h-12';
